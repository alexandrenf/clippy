...
pub fn create(app: &AppHandle) -> tauri::Result<()> {
    let show = MenuItem::with_id(app, "show", "Show / Hide Clippy", true, None::<&str>)?;
    ...
    let tray_icon = app.default_window_icon().expect("app icon missing").clone();

    let autostart_item = autostart.clone();
    let tray = TrayIconBuilder::with_id("clippy-tray")
        .icon(tray_icon)
        .tooltip("Clippy — double-tap Right Shift to open")
        .menu(&menu)
    ...
    tray.on_menu_event(move |app, event| match event.id().as_ref() {
        ...
        "autostart" => {
            ...
            if let Err(e) = result {
                eprintln!("clippy: autostart toggle failed: {e}");
            }
